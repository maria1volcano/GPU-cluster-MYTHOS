...
from sentinel.models import Evidence, Prediction
...
def _thermal_prediction():
    return Prediction(
        ...
        target={"kind": "rack", "id": "rack-000"},
        eta_seconds=200.0,
        severity="high",
        confidence=0.8,
        evidence=[Evidence(metric="rack_temp_c", slope_per_min=2.0, threshold=83.0, current=70.0)],
        t=0,
    )
...
def test_repeated_overrides_tighten_the_lead_time_threshold(tmp_path):
    ...
    assert adjustments, "expected an adjustment after repeated overrides"
