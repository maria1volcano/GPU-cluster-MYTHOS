"""Crusoe Inference client — DESIGN.md §2.5.

Thin wrapper around the OpenAI-compatible Crusoe Managed Inference
endpoint. The agent (`Agent.recommend`) only ever asks this client to
*choose and justify* among already-validated candidates (see
`sentinel.agent.recommender.Recommender`) — it never lets the model invent
a job id or target rack, per the DESIGN §2.5 guardrail.

Any failure (missing API key, timeout, malformed JSON, network error)
returns `None` so the caller can degrade gracefully to the templated
fallback (NFR-5).
"""
...
class CrusoeClient:
    ...
    def choose_candidate(self, prediction: Prediction, candidates: List[Candidate]) -> Optional[Dict[str, Any]]:
        """Returns `{"candidate_index": int, "justification": str}` or `None`
        on any failure — callers must fall back to a templated
        recommendation (NFR-5: graceful degradation)."""
