"""Learning from overrides — M7 (DESIGN.md §2.7)."""
...
class OverrideLearner:
    ...
    def _approved_averted_rate(group: List[dict]) -> float:
        hits = sum(
            1 for e in group if e.get("operator_action") == "APPROVE" and e.get("outcome") == "AVERTED"
        )
    ...
    def _adjust_bottleneck(self, group: List[dict], override_rate: float, averted_rate: float):
        ...
        if override_rate >= config.LEARNING_OVERRIDE_RATE_HIGH:
            ...
            direction = "tightened"
        elif averted_rate >= 0.5 and (
            old_util > config.DEFAULT_BOTTLENECK_UTIL_THRESHOLD
            or old_heavy > config.DEFAULT_BOTTLENECK_QUEUED_HEAVY
        ):
